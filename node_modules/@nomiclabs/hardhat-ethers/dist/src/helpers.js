...
exports.getContractAt = exports.getContractFactoryByAbiAndBytecode = exports.getContractFactory = exports.getSigners = void 0;
...
async function getContractFactory(hre, nameOrAbi, bytecodeOrFactoryOptions, signer) {
    if (typeof nameOrAbi === "string") {
        const contractFactory = await getContractFactoryByName(hre, nameOrAbi, bytecodeOrFactoryOptions);
        if (contractFactory.bytecode === "0x") {
            throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `You are trying to create a contract factory for the contract ${nameOrAbi}, which is abstract and can't be deployed.
...
async function getContractFactoryByName(hre, contractName, signerOrOptions) {
    ...
    for (const [linkedLibraryName, linkedLibraryAddress] of Object.entries(libraries)) {
        if (!utils.isAddress(linkedLibraryAddress)) {
            throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `You tried to link the contract ${contractName} with the library ${linkedLibraryName}, but provided this invalid address: ${linkedLibraryAddress}`);
        }
...
${libraryFQNames}`;
            ...
            throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `You tried to link the contract ${contractName} with ${linkedLibraryName}, which is not one of its libraries.
${detailedMessage}`);
        }
        if (matchingNeededLibraries.length > 1) {
            const matchingNeededLibrariesFQNs = matchingNeededLibraries
                ...
                .map((x) => `* ${x}`)
                .join("\n");
            throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `The library name ${linkedLibraryName} is ambiguous for the contract ${contractName}.
...
Remove one of them and review your library links before proceeding.`);
    ...
    if (linksToApply.size < neededLibraries.length) {
        const missingLibraries = neededLibraries
            ...
            .map((x) => `* ${x}`)
            .join("\n");
        throw new plugins_1.NomicLabsHardhatPluginError(pluginName, `The contract ${contractName} is missing links for the following libraries:
${missingLibraries}`);
    }
