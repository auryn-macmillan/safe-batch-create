...
function filterLogs(logs, criteria) {
    ...
    for (const log of logs) {
        const blockNumber = new ethereumjs_util_1.BN(ethereumjs_util_1.toBuffer(log.blockNumber));
        ...
        if (criteria.addresses.length !== 0 &&
            !includes(criteria.addresses, ethereumjs_util_1.toBuffer(log.address))) {
            continue;
        }
...
function topicMatched(normalizedTopics, logTopics) {
    for (let i = 0; i < normalizedTopics.length; i++) {
        ...
        let match = false;
        for (const topic of sub) {
            if (topic === null || logTopics[i] === ethereumjs_util_1.bufferToHex(topic)) {
                match = true;
                break;
            }
        }
