...
class ForkBlockchain {
    ...
    async getLogs(filterParams) {
        if (filterParams.fromBlock.lte(this._forkBlockNumber)) {
            ...
            return remoteLogs.map((log) => output_1.toRpcLogOutput(log)).concat(localLogs);
        }
    ...
    async _processRemoteBlock(rpcBlock) {
        ...
        const block = new Block_1.Block(rpcToBlockData_1.rpcToBlockData(rpcBlock), { common: this._common });
    ...
    _processRemoteTransaction(rpcTransaction) {
        ...
        const transaction = new ethereumjs_tx_1.Transaction(rpcToTxData_1.rpcToTxData(rpcTransaction), {
            common: this._common,
        });
