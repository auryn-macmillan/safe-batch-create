...
const putGenesisAccounts_1 = require("./utils/putGenesisAccounts");
...
class HardhatNode extends events_1.default {
    ...
    static async create(config) {
        ...
        if (config.type === "forked") {
            ...
            common = await makeForkCommon_1.makeForkCommon(forkClient, forkBlockNumber);
            stateManager = new ForkStateManager_1.ForkStateManager(forkClient, forkBlockNumber);
            await putGenesisAccounts_1.putGenesisAccounts(stateManager, genesisAccounts);
    ...
    async runCall(call, blockNumber) {
        ...
        const result = await this._runInBlockContext(blockNumber, () => this._runTxAndRevertMutations(tx, blockNumber === null));
        let vmTrace = this._vmTracer.getLastTopLevelMessageTrace();
        const vmTracerError = this._vmTracer.getLastError();
        this._vmTracer.clearLastError();
        if (vmTrace !== undefined) {
