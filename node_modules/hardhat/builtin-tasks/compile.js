...
const unsafe_1 = require("../internal/util/unsafe");
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_GET_DEPENDENCY_GRAPH)
    ...
    .setAction(async ({ sourceNames, solidityFilesCache, }, { config }) => {
    const parser = new parse_1.Parser(solidityFilesCache);
    const resolver = new resolver_1.Resolver(config.paths.root, parser);
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOBS)
    ...
    log(`The dependency graph was dividied in '${connectedComponents.length}' connected components`);
    ...
    const compilationJobsCreationResult = compilationJobsCreationResults.reduce((acc, { jobs, errors }) => {
        var _a;
        acc.jobs = acc.jobs.concat(jobs);
        for (const [code, files] of unsafe_1.unsafeObjectEntries(errors)) {
            acc.errors[code] = (_a = acc.errors[code]) !== null && _a !== void 0 ? _a : [];
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_LOG_DOWNLOAD_COMPILER_START)
    ...
    .setAction(async ({ isCompilerDownloaded, quiet, solcVersion, }) => {
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_EMIT_ARTIFACTS)
    ...
    .setAction(async ({ compilationJob, input, output, solcBuild, }, { artifacts, config, run }) => {
    var _a, _b;
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_COMPILE_JOB)
    ...
    .addParam("compilationJobIndex", undefined, undefined, config_env_1.types.int)
    .addParam("quiet", undefined, undefined, config_env_1.types.boolean)
    .setAction(async ({ compilationJob, compilationJobs, compilationJobIndex, quiet, }, { run }) => {
    ...
    const { artifactsEmittedPerFile } = await run(task_names_1.TASK_COMPILE_SOLIDITY_EMIT_ARTIFACTS, {
        compilationJob,
        input,
        output,
        solcBuild,
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_HANDLE_COMPILATION_JOBS_FAILURES)
    ...
    const hasErrors = unsafe_1.unsafeObjectEntries(compilationJobsCreationErrors).some(([, errors]) => errors.length > 0);
    if (hasErrors) {
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOBS_FAILURE_REASONS)
    ...
    let noCompatibleSolc = [];
    let incompatibleOverridenSolc = [];
    let importsIncompatibleFile = [];
    let other = [];
    for (const code of unsafe_1.unsafeObjectKeys(errors)) {
        const files = errors[code];
        if (files === undefined) {
            continue;
        }
        if (code === builtin_tasks_1.CompilationJobCreationError.NO_COMPATIBLE_SOLC_VERSION_FOUND) {
        ...
        else if (code === builtin_tasks_1.CompilationJobCreationError.IMPORTS_INCOMPATIBLE_FILE) {
            importsIncompatibleFile = importsIncompatibleFile.concat(files);
        }
        else if (code === builtin_tasks_1.CompilationJobCreationError.OTHER_ERROR) {
...
function needsCompilation(job, cache) {
    for (const file of job.getResolvedFiles()) {
        const hasChanged = cache.hasFileChanged(file.absolutePath, file.lastModificationDate, 
